...
(function (global, factory) {
  ...
  class GoogleMapsProvider extends MapProvider {
    ...
    createSession() {
      const address =
        "https://www.googleapis.com/tile/v1/createSession?key=" + this.apiToken;
      ...
      return new Promise((resolve, reject) => {
        ...
        if (this.mode === MapBoxProvider.STYLE) {
          image.src =
            ...
            "/" +
            y +
            (this.useHDPI ? "@2x?access_token=" : "?access_token=") +
            this.apiToken;
        } else {
          image.src =
            MapBoxProvider.ADDRESS +
  ...
  class DebugProvider extends MapProvider {
    ...
    fetchTile(zoom, x, y) {
      ...
      const color = green.lerp(
        red,
        (zoom - this.minZoom) / (this.maxZoom - this.minZoom)
      );
        ...
        this.provider
          ...
          .then((image) => {
            ...
            for (let i = 0; i < data.length; i += 4) {
              ...
              const max = 1667721.6;
              const color = this.fromColor
                .clone()
                .lerpHSL(this.toColor, value / max);
              data[i] = color.r * 255;
